...
def get_recent_exec_movements(db_path: str) -> list:
    """Get executive movements detected in the past 7 days."""
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    ...
    cur.execute("""
        ...
        WHERE c.notes LIKE '%Moved from%'
        AND c.updated_at >= date('now', '-7 days')
        ORDER BY c.updated_at DESC
        LIMIT 10
    """)
    movements = [dict(row) for row in cur.fetchall()]
    conn.close()
    return movements
...
def generate_weekly_digest(db_path: str = None) -> str:
    ...
    exec_moves = get_recent_exec_movements(db_path)
    ...
    if exec_moves:
        exec_lines = "\n".join(
            f"  - {m['name']} ({m['title']}) → {m['current_company']}"
            + (f" (from {m['previous_companies']})" if m['previous_companies'] else "")
            for m in exec_moves
        )
    else:
        exec_lines = "  No movements detected this week"
